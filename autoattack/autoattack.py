...
class AutoAttack:
        ...
        if self.verbose:
            self.logger.log(
                "using {} version including {}.".format(
                    self.version, ", ".join(attacks_to_run)
                )
            )
            if state.run_attacks:
                self.logger.log(
                    "{} was/were already run.".format(", ".join(state.run_attacks))
                )
        ...
        with torch.no_grad():
            ...
            if state.robust_flags is None:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # TODO
                    # To get the flags for correct and wrong predictions
                    # As in the attacks, we will only attack correct predictions
                    robust_flags[start_idx:end_idx] = correct_batch.detach().to(
                        robust_flags.device
                    )

                # After all batches, i.e., the forward process for all the data
                # TODO: setting robust_flags to select correctly predicted examples
                ...
                if self.verbose:
                    self.logger.log(
                        "initial clean accuracy: {:.2%}".format(state.clean_accuracy)
                    )
                    self.logger.log(
                        "robust accuracy "
                        "at the time of restoring the state: {:.2%}".format(
                            robust_accuracy
                        )
                    )
            ...
            for attack in attacks_to_run:
                ...
                # TODO, how robust_flags is updated after each attack?
                # TODO, actually, robust_flags is updated per batch within each attack
                # As in the next attack, we will only attack:
                #   correct predictions
                #   which have not be successfully attacked
                # When there is no correct predictions to be attacked,
                #   we stop the future attacks
                if num_robust == 0:
                    break

                n_batches = int(np.ceil(num_robust / bs))

                # Get the global indexes of
                #     TODO: correct predictions which have not be successfully attacked
                ...
                for batch_idx in range(n_batches):
                    ...
                    # make sure that x is a 4d tensor even
                    #   if there is only a single datapoint left
                    if len(x.shape) == 3:
                        x.unsqueeze_(dim=0)
                    ...
                    # when not equal, denoting successfully attacked
                    false_batch = ~y.eq(output).to(robust_flags.device)

                    # get the global indexes of non_equal = non_robust any more
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
                    # TODO: updating robust_flags
                    # TODO: set the robust flags to False
                    #   for those being successfully attacked
                    robust_flags[non_robust_lin_idcs] = False
                    # sync state.robust_flags with robust_flags
                    state.robust_flags = robust_flags

                    # TODO: update x_adv and y_adv for non-robust data points
                    x_adv[non_robust_lin_idcs] = (
                        adv_curr[false_batch].detach().to(x_adv.device)
                    )
                    y_adv[non_robust_lin_idcs] = (
                        output[false_batch].detach().to(x_adv.device)
                    )

                    # verbose information for each batch
                    if self.verbose:
                ...
                # ##########################################
                # calculate the new accuracy after each attack
                robust_accuracy = torch.sum(robust_flags).item() / x_orig.shape[0]
            ...
            if self.verbose:
                if self.norm == "Linf":
                    ...
                    res = (
                        ((x_adv - x_orig) ** 2)
                        .reshape(x_orig.shape[0], -1)
                        .sum(-1)
                        .sqrt()
                    )
                elif self.norm == "L1":
                    res = (
                        (x_adv - x_orig).abs().reshape(x_orig.shape[0], -1).sum(dim=-1)
                    )
                self.logger.log(
                    "max {} perturbation: {:.5f}, "
                    "nan in tensor: {}, max: {:.5f}, min: {:.5f}".format(
                        self.norm,
                        res.max(),
                        (x_adv != x_adv).sum(),
                        x_adv.max(),
    ...
    def run_standard_evaluation_individual(
        self, x_orig, y_orig, bs=250, return_labels=False
    ):
        if self.verbose:
            print(
                "using {} version including {}".format(
        ...
        for c in l_attacks:
            ...
            x_adv, y_adv = self.run_standard_evaluation(
                x_orig, y_orig, bs=bs, return_labels=True
            )
            if return_labels:
                adv[c] = (x_adv, y_adv)
            else:
